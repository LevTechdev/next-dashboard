...
import { TreeSitterConfigSchema, FilePatternConfigSchema, LanguageConfigSchema, StrictLanguageConfigSchema, FrameworkConfigSchema, } from "../languages/types.js";
...
function countConfigModules(relativeDir) {
...
describe("Built-in Language Configs", () => {
    ...
    it("registers every config module in the configs directory", () => {
        expect(builtinLanguageConfigs).toHaveLength(countConfigModules("../languages/configs/"));
    });
    it("every config passes base LanguageConfigSchema validation", () => {
        for (const config of builtinLanguageConfigs) {
            ...
            expect(result.success, `"${config.id}" should pass base LanguageConfigSchema: ${result.success ? "" : result.error.issues.map((i) => i.message).join(", ")}`).toBe(true);
        }
    });
    it("content-detected configs intentionally fail StrictLanguageConfigSchema", () => {
        for (const config of builtinLanguageConfigs) {
            if (!CONTENT_DETECTED_IDS.has(config.id))
                continue;
            const result = StrictLanguageConfigSchema.safeParse(config);
            expect(result.success, `"${config.id}" is content-detected (no extensions/filenames) and should fail strict validation by design`).toBe(false);
        }
    });
    it("all non-content-detected configs pass StrictLanguageConfigSchema", () => {
        for (const config of builtinLanguageConfigs) {
            if (CONTENT_DETECTED_IDS.has(config.id))
                continue;
            const result = StrictLanguageConfigSchema.safeParse(config);
            expect(result.success, `"${config.id}" should pass StrictLanguageConfigSchema: ${result.success ? "" : JSON.stringify(result.error.issues)}`).toBe(true);
        }
    });
    ...
    it("every config has at least one concept", () => {
        for (const config of builtinLanguageConfigs) {
            expect(config.concepts.length, `"${config.id}" should have at least one concept`).toBeGreaterThan(0);
        }
    });
    ...
    it("configs with treeSitter have valid wasmPackage and wasmFile", () => {
        for (const config of builtinLanguageConfigs) {
            ...
            expect(tsResult.success, `"${config.id}" treeSitter should be valid: ${tsResult.success ? "" : JSON.stringify(tsResult.error.issues)}`).toBe(true);
        }
    });
    it("configs with filenames have at least one entry", () => {
        for (const config of builtinLanguageConfigs) {
            if (!config.filenames)
                continue;
            expect(config.filenames.length, `"${config.id}" has filenames field but it is empty`).toBeGreaterThan(0);
        }
    });
});
// =============================================================================
...
describe("Built-in Framework Configs", () => {
    it("registers every framework module in the frameworks directory", () => {
        expect(builtinFrameworkConfigs).toHaveLength(countConfigModules("../languages/frameworks/"));
    });
    it("every framework config passes FrameworkConfigSchema validation", () => {
        for (const fw of builtinFrameworkConfigs) {
            ...
            expect(result.success, `"${fw.id}" should pass FrameworkConfigSchema: ${result.success ? "" : result.error.issues.map((i) => i.message).join(", ")}`).toBe(true);
        }
    });
    ...
    it("every framework's languages array references known language ids", () => {
        ...
        for (const fw of builtinFrameworkConfigs) {
            for (const langId of fw.languages) {
                expect(knownLanguageIds.has(langId), `"${fw.id}" references unknown language "${langId}"`).toBe(true);
            }
        }
    });
    it("every framework has at least one detectionKeyword and manifestFile", () => {
        for (const fw of builtinFrameworkConfigs) {
            expect(fw.detectionKeywords.length, `"${fw.id}" should have at least one detection keyword`).toBeGreaterThan(0);
            expect(fw.manifestFiles.length, `"${fw.id}" should have at least one manifest file`).toBeGreaterThan(0);
        }
    });
    it("every framework has a non-empty promptSnippetPath", () => {
        for (const fw of builtinFrameworkConfigs) {
            expect(fw.promptSnippetPath.length, `"${fw.id}" should have a non-empty promptSnippetPath`).toBeGreaterThan(0);
        }
    });
    it("frameworks with layerHints have valid string key-value pairs", () => {
        for (const fw of builtinFrameworkConfigs) {
            ...
            expect(entries.length, `"${fw.id}" layerHints should have at least one entry`).toBeGreaterThan(0);
