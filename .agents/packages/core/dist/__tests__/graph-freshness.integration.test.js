...
import { getGraphFreshness, getGraphFreshnessBatch, } from "../staleness.js";
...
function createTemporaryDirectory(prefix = "ua-freshness-") {
    const directory = mkdtempSync(join(tmpdir(), prefix));
    temporaryDirectories.push(directory);
    return directory;
}
function writeProjectFile(projectDir, relativePath, contents) {
...
function createRepository(initialFiles = {
...
describe("getGraphFreshness with a real Git repository", { timeout: 15_000 }, () => {
    it("returns fresh for a clean project at the analyzed commit", async () => {
        const { repoDir, baseline } = createRepository();
        await expect(getGraphFreshness(repoDir, {
    ...
    it.each([
        ...
        },
        {
            ...
            prepare(repoDir) {
                writeProjectFile(repoDir, "src/untracked.ts", "export const untracked = true;\n");
            },
        },
    ])("returns dirty for $name project changes", async ({ path, prepare }) => {
        const { repoDir, baseline } = createRepository();
        prepare(repoDir);
        await expect(getGraphFreshness(repoDir, { graphCommitHash: baseline })).resolves.toMatchObject({
            status: "dirty",
            graphCommitHash: baseline,
            headCommitHash: baseline,
            changedFileCount: 1,
    ...
    it.each([".understand-anything", ".ua"])("ignores untracked Understand Anything output files in %s", async (dataDir) => {
        const { repoDir, baseline } = createRepository();
        writeProjectFile(repoDir, `${dataDir}/knowledge-graph.json`, "{}\n");
        writeProjectFile(repoDir, `${dataDir}/intermediate/batch-0.json`, "{}\n");
        await expect(getGraphFreshness(repoDir, { graphCommitHash: baseline })).resolves.toMatchObject({
            status: "fresh",
            changedFileCount: 0,
            changedFiles: [],
    ...
    it.each([".understand-anything", ".ua"])("ignores commits that only change Understand Anything output files in %s", async (dataDir) => {
        const { repoDir, baseline } = createRepository();
        writeProjectFile(repoDir, `${dataDir}/knowledge-graph.json`, "{}\n");
        const headCommit = commitAll(repoDir, "persist generated graph");
        await expect(getGraphFreshness(repoDir, { graphCommitHash: baseline })).resolves.toEqual({
            status: "fresh",
            graphCommitHash: baseline,
    ...
    it.each([".understand-anything", ".ua"])("still reports source changes beside ignored output files in %s", async (dataDir) => {
        const { repoDir, baseline } = createRepository();
        writeProjectFile(repoDir, `${dataDir}/knowledge-graph.json`, "{}\n");
    ...
    it("reports a graph ahead of the checked-out project history", async () => {
        ...
        await expect(getGraphFreshness(repoDir, { graphCommitHash: graphCommit })).resolves.toMatchObject({
            status: "stale",
            relation: "ahead",
    ...
    it("ignores commits that only touch a sibling monorepo project", async () => {
        ...
        writeProjectFile(repoDir, "apps/sibling/src/index.ts", "export const sibling = 2;\n");
        const head = commitAll(repoDir, "sibling-only change");
        const targetProject = join(repoDir, "apps", "target");
        await expect(getGraphFreshness(targetProject, { graphCommitHash: baseline })).resolves.toEqual({
            status: "fresh",
            graphCommitHash: baseline,
            headCommitHash: head,
            changedFileCount: 0,
    ...
    it("ignores staged, unstaged, and untracked changes in a sibling project", async () => {
        ...
        writeProjectFile(repoDir, "apps/sibling/src/staged.ts", "export const staged = 2;\n");
        git(repoDir, "add", "apps/sibling/src/staged.ts");
        writeProjectFile(repoDir, "apps/sibling/src/unstaged.ts", "export const unstaged = 2;\n");
        writeProjectFile(repoDir, "apps/sibling/src/untracked.ts", "export const untracked = true;\n");
        const targetProject = join(repoDir, "apps", "target");
        await expect(getGraphFreshness(targetProject, { graphCommitHash: baseline })).resolves.toEqual({
            status: "fresh",
            graphCommitHash: baseline,
    ...
    it("counts only selected-project commits and returns project-relative paths", async () => {
        ...
        writeProjectFile(repoDir, "apps/target/src/index.ts", "export const target = 2;\n");
        commitAll(repoDir, "target-project change");
        const targetProject = join(repoDir, "apps", "target");
        await expect(getGraphFreshness(targetProject, { graphCommitHash: baseline })).resolves.toMatchObject({
            status: "stale",
            relation: "behind",
            changedFileCount: 1,
            changedFiles: ["src/index.ts"],
    ...
    it("evaluates multiple graph commits in one batch", async () => {
        ...
        await expect(getGraphFreshnessBatch(repoDir, {
    ...
    it("returns missing-graph-commit without consulting Git", async () => {
        ...
        await expect(getGraphFreshness(projectDir, {
            graphCommitHash: "  ",
            lastAnalyzedAt: "2026-07-10T00:00:00.000Z",
        })).resolves.toEqual({
            status: "unknown",
    ...
    it("returns git-head-unavailable outside a Git repository", async () => {
        const projectDir = createTemporaryDirectory();
        await expect(getGraphFreshness(projectDir, { graphCommitHash: "deadbeef" })).resolves.toEqual({
            status: "unknown",
    ...
    it("returns graph-commit-unavailable for an unknown graph commit", async () => {
        const { repoDir, baseline } = createRepository();
        await expect(getGraphFreshness(repoDir, { graphCommitHash: "deadbeef" })).resolves.toEqual({
            status: "unknown",
...
//# sourceMappingURL=graph-freshness.integration.test.js.map