...
import { validateGraph, sanitizeGraph, autoFixGraph, NODE_TYPE_ALIASES, EDGE_TYPE_ALIASES, } from "../schema.js";
const validGraph = {
    version: "1.0.0",
    project: {
        name: "test-project",
...
describe("schema validation", () => {
    ...
    it("rejects node with invalid type — drops node, fatal if none remain", () => {
        ...
        expect(result.issues).toContainEqual(expect.objectContaining({ level: "dropped", category: "invalid-node" }));
    });
    it("drops edge with invalid EdgeType but loads graph", () => {
        ...
        expect(result.issues).toContainEqual(expect.objectContaining({ level: "dropped", category: "invalid-edge" }));
    });
    it("auto-corrects weight >1 by clamping", () => {
        ...
        const result = validateGraph(graph);
        expect(result.success).toBe(true);
        expect(result.issues).toContainEqual(expect.objectContaining({ level: "auto-corrected", category: "out-of-range" }));
    });
    ...
    it('drops "tests" edge type — direction-inverting alias is unsafe', () => {
        ...
        expect(result.success).toBe(true);
        expect(result.data.edges.length).toBe(0);
        expect(result.issues).toContainEqual(expect.objectContaining({ level: "dropped" }));
    });
    ...
    it("NODE_TYPE_ALIASES values are never alias keys (no chains)", () => {
        for (const [alias, target] of Object.entries(NODE_TYPE_ALIASES)) {
            expect(NODE_TYPE_ALIASES, `chain detected: ${alias} → ${target} → ${NODE_TYPE_ALIASES[target]}`).not.toHaveProperty(target);
        }
    });
    it("EDGE_TYPE_ALIASES values are never alias keys (no chains)", () => {
        for (const [alias, target] of Object.entries(EDGE_TYPE_ALIASES)) {
            expect(EDGE_TYPE_ALIASES, `chain detected: ${alias} → ${target} → ${EDGE_TYPE_ALIASES[target]}`).not.toHaveProperty(target);
        }
    });
});
describe("sanitizeGraph", () => {
    it("converts null optional node fields to undefined", () => {
        const graph = structuredClone(validGraph);
        graph.nodes[0].filePath = null;
        graph.nodes[0].lineRange = null;
        graph.nodes[0].languageNotes = null;
    ...
    it("converts null optional edge fields to undefined", () => {
        const graph = structuredClone(validGraph);
        graph.edges[0].description = null;
        const result = sanitizeGraph(graph);
    ...
    it("converts null optional tour step fields to undefined", () => {
        const graph = structuredClone(validGraph);
        graph.tour[0].languageLesson = null;
        const result = sanitizeGraph(graph);
        expect(result.tour[0].languageLesson).toBeUndefined();
    });
    it("passes through non-object node/edge items unchanged", () => {
        const graph = { nodes: [null, "garbage", 42], edges: [null], tour: [], layers: [] };
        const result = sanitizeGraph(graph);
        expect(result.nodes).toEqual([null, "garbage", 42]);
        expect(result.edges).toEqual([null]);
    });
});
describe("autoFixGraph", () => {
    it("defaults missing complexity to moderate with issue", () => {
        const graph = structuredClone(validGraph);
        delete graph.nodes[0].complexity;
        const { data, issues } = autoFixGraph(graph);
        expect(data.nodes[0].complexity).toBe("moderate");
        expect(issues).toContainEqual(expect.objectContaining({ level: "auto-corrected", category: "missing-field", path: "nodes[0].complexity" }));
    });
    it("maps complexity aliases with issue", () => {
        const graph = structuredClone(validGraph);
        graph.nodes[0].complexity = "low";
        const { data, issues } = autoFixGraph(graph);
        expect(data.nodes[0].complexity).toBe("simple");
    ...
    it("maps all complexity aliases correctly", () => {
        ...
        for (const [alias, expected] of Object.entries(mapping)) {
            ...
            const { data } = autoFixGraph(graph);
            expect(data.nodes[0].complexity).toBe(expected);
        }
    });
    it("defaults missing tags to empty array with issue", () => {
        const graph = structuredClone(validGraph);
        delete graph.nodes[0].tags;
        const { data, issues } = autoFixGraph(graph);
        expect(data.nodes[0].tags).toEqual([]);
        expect(issues).toContainEqual(expect.objectContaining({ level: "auto-corrected", category: "missing-field", path: "nodes[0].tags" }));
    });
    it("defaults missing summary to node name with issue", () => {
        const graph = structuredClone(validGraph);
        delete graph.nodes[0].summary;
        const { data, issues } = autoFixGraph(graph);
        expect(data.nodes[0].summary).toBe("index.ts");
        expect(issues).toContainEqual(expect.objectContaining({ level: "auto-corrected", category: "missing-field", path: "nodes[0].summary" }));
    });
    it("defaults missing node type to file with issue", () => {
        const graph = structuredClone(validGraph);
        delete graph.nodes[0].type;
        const { data, issues } = autoFixGraph(graph);
        expect(data.nodes[0].type).toBe("file");
        expect(issues).toContainEqual(expect.objectContaining({ level: "auto-corrected", category: "missing-field", path: "nodes[0].type" }));
    });
    it("defaults missing direction to forward with issue", () => {
        const graph = structuredClone(validGraph);
        delete graph.edges[0].direction;
        const { data, issues } = autoFixGraph(graph);
        expect(data.edges[0].direction).toBe("forward");
        expect(issues).toContainEqual(expect.objectContaining({ level: "auto-corrected", category: "missing-field", path: "edges[0].direction" }));
    });
    it("maps direction aliases with issue", () => {
        ...
        for (const [alias, expected] of Object.entries(mapping)) {
            ...
            const { data } = autoFixGraph(graph);
            expect(data.edges[0].direction).toBe(expected);
        }
    });
    it("defaults missing weight to 0.5 with issue", () => {
        const graph = structuredClone(validGraph);
        delete graph.edges[0].weight;
        const { data, issues } = autoFixGraph(graph);
        expect(data.edges[0].weight).toBe(0.5);
        expect(issues).toContainEqual(expect.objectContaining({ level: "auto-corrected", category: "missing-field", path: "edges[0].weight" }));
    });
    it("coerces string weight to number with issue", () => {
        ...
        expect(issues).toContainEqual(expect.objectContaining({ level: "auto-corrected", category: "type-coercion", path: "edges[0].weight" }));
    });
    it("clamps out-of-range weight with issue", () => {
        ...
        expect(issues).toContainEqual(expect.objectContaining({ level: "auto-corrected", category: "out-of-range", path: "edges[0].weight" }));
    });
    it("defaults missing edge type to depends_on with issue", () => {
        const graph = structuredClone(validGraph);
        delete graph.edges[0].type;
        const { data, issues } = autoFixGraph(graph);
        expect(data.edges[0].type).toBe("depends_on");
        expect(issues).toContainEqual(expect.objectContaining({ level: "auto-corrected", category: "missing-field", path: "edges[0].type" }));
    });
    it("returns no issues for a valid graph", () => {
        const { issues } = autoFixGraph(validGraph);
        expect(issues).toEqual([]);
    });
    it("passes through non-object node/edge items unchanged", () => {
        const graph = { nodes: [null, "garbage"], edges: [null], tour: [], layers: [] };
        const { data, issues } = autoFixGraph(graph);
        expect(data.nodes).toEqual([null, "garbage"]);
        expect(data.edges).toEqual([null]);
        expect(issues).toEqual([]);
    });
});
describe("permissive validation", () => {
    ...
    it("drops edges referencing non-existent nodes with dropped issue", () => {
        ...
        expect(result.success).toBe(true);
        expect(result.data.edges.length).toBe(0);
        expect(result.issues).toContainEqual(expect.objectContaining({ level: "dropped", category: "invalid-reference" }));
    });
    ...
    it("loads graph with mixed good and bad nodes", () => {
        ...
        graph.nodes.push({ type: "file", summary: "broken" });
        const result = validateGraph(graph);
        expect(result.success).toBe(true);
    ...
    it("handles non-parseable string weight by defaulting to 0.5", () => {
        ...
        expect(result.issues).toContainEqual(expect.objectContaining({ level: "auto-corrected", category: "type-coercion" }));
    });
