...
describe("getChangedFiles", () => {
    it("returns changed file list from git diff", () => {
        ...
        expect(mockedExecFileSync).toHaveBeenCalledWith("git", ["diff", "abc123..HEAD", "--name-only"], { cwd: "/project", encoding: "utf-8" });
    });
...
describe("mergeGraphUpdate", () => {
    it("replaces nodes for changed files", () => {
        ...
        const result = mergeGraphUpdate(existingGraph, ["src/a.ts"], newNodes, [], "def456");
    ...
    it("removes edges originating from changed files", () => {
        ...
        const result = mergeGraphUpdate(existingGraph, ["src/a.ts"], newNodes, newEdges, "def456");
        // Old edge from file-a should be removed
        expect(result.edges.find((e) => e.source === "file-a" && e.target === "file-b")).toBeUndefined();
        // Edge between unchanged files should remain
        expect(result.edges.find((e) => e.source === "file-b" && e.target === "file-c")).toBeDefined();
        // Edge to changed file from unchanged should be removed (dangling target)
        expect(result.edges.find((e) => e.source === "file-c" && e.target === "file-a")).toBeUndefined();
        // New edge should be added
        expect(result.edges.find((e) => e.source === "file-a-v2" && e.target === "file-c")).toBeDefined();
    });
