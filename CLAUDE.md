...
### 11. Mail Delivery — Durable Outbox & the Sandbox-Sender Trap

- **Transactional mail is queued, never sent inline.** `src/lib/email-outbox.ts` writes an `EmailOutbox` row _before_ any transport is touched; delivery runs off the response path (`after()` inside a request, the `email-outbox` scheduler job otherwise) with exponential backoff, so a slow SMTP handshake can no longer be killed by a function timeout with the message lost. `verify_email` rows are re-issued at delivery time (fresh code + new hash) — a one-time code is never stored.
- **Routes must distinguish "sent" from "queued".** Responses carry `emailSent` (a transport already accepted it) and `emailQueued` (durable, will be retried), plus `mailMisconfigured: true` when `describeMailConfiguration().warnings` is non-empty. The UI warns on `mailMisconfigured` instead of promising an inbox message; never print a one-time code to the server log when a mailer is configured.
- **`onboarding@resend.dev` is not a usable sender.** Resend's sandbox address only delivers to the account owner, so the mailer rescues such a send to SMTP (and sends as the authenticated SMTP account, since Gmail rewrites a From it does not own) and logs the fix. Never "solve" a mail problem by pointing the sender back at it.
- `npm run verify:mail -- you@example.com` prints the resolved transport/sender/warnings and performs one real send (local only; it refuses to run in production). `src/lib/email.ts` classifies failures with `isTransientMailError` — extend that, not the retry loop, when a new provider error needs different handling.
