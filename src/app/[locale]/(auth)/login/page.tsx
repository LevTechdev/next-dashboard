...
import { Smartphone, Sparkles, KeyRound, ChevronLeft, ChevronRight, Fingerprint, Building2, Sun, Moon } from "lucide-react";
      ...
      <div className="w-full max-w-[1000px] bg-white dark:bg-zinc-900 dark:bg-zinc-900 rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row min-h-[600px] border border-white/20 dark:border-zinc-800 transition-colors">
        ...
        <div className="flex-1 p-8 sm:p-12 flex flex-col justify-center">
          <div className="w-full max-w-sm mx-auto">
            ...
            {totpRequired ? (
              <>
                <h1 className="text-2xl sm:text-3xl font-bold text-zinc-900 mb-3">Two-Factor Auth</h1>
                    ...
                    <div className="relative flex justify-between gap-2 w-full">
                      {Array.from({ length: 6 }).map((_, i) => (
                        <div key={i} className={cn(
                          ...
                          totpCode.length === i ? "border-[#F25C38] ring-1 ring-[#F25C38]" : "border-zinc-200",
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div>
                  <h1 className="text-3xl font-bold text-zinc-900 dark:text-white mb-2">{t("forgotPasswordTitle")}</h1>
                  <p className="text-zinc-500 dark:text-zinc-400 text-sm">{t("forgotPasswordSubtitle")}</p>
                </div>
                
                <form onSubmit={async (e) => { e.preventDefault(); toast.success(t("resetLinkSent")); setView('login'); }} className="space-y-5">
                  <div>
                    <Label className="text-zinc-700 dark:text-zinc-300 font-medium mb-1.5 block">{t("email")}</Label>
                    <Input 
                  ...
                  <Button type="submit" disabled={isLoading} className="w-full h-11 rounded-xl bg-[#F5A898] hover:bg-[#EE5D36] transition-colors text-white font-semibold mt-2 shadow-none">
                    {isLoading ? <LoaderCircleIcon size={16} className="animate-spin" /> : t("sendResetLink")}
                  </Button>
                </form>
                
                <div className="text-center pt-4">
                  <button type="button" onClick={() => setView('login')} className="inline-flex items-center text-sm font-medium text-zinc-500 hover:text-[#EE5D36] dark:text-zinc-400 dark:hover:text-[#EE5D36] transition-colors">
                    Back to log in
                  </button>
                </div>
              </div>
                ...
                <>
                  <h1 className="text-2xl sm:text-3xl font-bold text-zinc-900 dark:text-white mb-3">Welcome back</h1>
                <p className="text-sm text-zinc-500 mb-8">
                ...
                <div className="flex gap-3 justify-center">
                  <button onClick={handleGoogleLogin} type="button" className="flex-1 h-12 border border-zinc-200 rounded-xl flex items-center justify-center hover:bg-zinc-50 transition-colors">
                  ...
                  <button type="button" className="flex-1 h-12 border border-zinc-200 rounded-xl flex items-center justify-center hover:bg-zinc-50 transition-colors">
