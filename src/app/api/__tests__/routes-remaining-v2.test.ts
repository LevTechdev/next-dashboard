...
const {
  ...
  const deepModel = <T extends Record<string, unknown>>(overrides: Partial<T> = {}) =>
    model({
      ...
      upsert: vi
        .fn()
        .mockImplementation(({ create }) => Promise.resolve({ id: "conn-1", ...create })),
      ...overrides,
    });
  ...
  return {
    ...
    mockPrisma: {
      ...
      ssoConnection: deepModel({
        ...
        upsert: vi
          .fn()
          .mockImplementation(({ create }) => Promise.resolve({ id: "sso-1", ...create })),
        deleteMany: vi.fn().mockResolvedValue({ count: 1 }),
      }),
...
vi.mock("@simplewebauthn/server", () => ({
  ...
  generateRegistrationOptions: vi
    .fn()
    .mockResolvedValue({ challenge: "reg-challenge", rp: { name: "NextDashboard" } }),
      ...
      mockPrisma.affiliateConversion.findMany.mockResolvedValueOnce([
        {
          ...
          link: {
            code: "CODE",
            product: { name: "W" },
            platform: { name: "T", slug: "t", color: "#000" },
          },
        },
      ]);
      const res = await conversionsRoutes.GET(mockRequest());
...
describe("Affiliates Import Link", () => {
  describe("GET", () => {
    ...
    it("returns preview product data", async () => {
      const res = await importLinkRoutes.GET(
        mockRequest(undefined, "url=https://www.tiktok.com/product/123"),
      );
      expect(res.status).toBe(200);
      const body = await res.json();
  ...
  describe("POST", () => {
    ...
    it("returns 400 for unsupported platform", async () => {
      const res = await importLinkRoutes.POST(
        mockRequest({ url: "https://www.amazon.com/product" }),
      );
      expect(res.status).toBe(400);
    });
    ...
    it("returns 404 when platform not configured", async () => {
      ...
      const res = await importLinkRoutes.POST(
        mockRequest({ url: "https://www.tiktok.com/product/123" }),
      );
      expect(res.status).toBe(404);
    });
  ...
  it("syncs products successfully", async () => {
    mockGetConnector.mockReturnValueOnce({
      fetchProducts: vi.fn().mockResolvedValue({
        ok: true,
        products: [{ name: "Product A", price: 10, stock: 5, sku: "SKU-A", externalId: "ext-a" }],
      }),
    });
    const res = await platformSyncRoutes.POST(mockRequest(), params);
    expect(res.status).toBe(200);
...
describe("SAML Connections", () => {
  ...
  describe("POST", () => {
    ...
    it("returns 400 when idpCert missing on first setup", async () => {
      ...
      const res = await samlConnectionsRoutes.POST(
        mockRequest({ name: "SSO", entryPoint: "https://idp.com" }),
      );
      expect(res.status).toBe(400);
    });
    ...
    it("preserves existing cert when not provided on update", async () => {
      mockPrisma.ssoConnection.findUnique.mockResolvedValueOnce({
        id: "sso-1",
        name: "Old",
        entryPoint: "https://old",
        idpCert: "existing-cert",
      });
      const res = await samlConnectionsRoutes.POST(
        mockRequest({ name: "New", entryPoint: "https://new" }),
      );
      expect(res.status).toBe(200);
    });
  });
  ...
  describe("PATCH", () => {
    ...
    it("updates connection", async () => {
      ...
      mockPrisma.ssoConnection.update.mockResolvedValueOnce({
        id: "sso-1",
        name: "New",
        enabled: true,
      });
...
describe("WebAuthn Register Verify", () => {
  it("returns 401 when not authenticated", async () => {
    ...
    const res = await webauthnRegVerifyRoutes.POST(
      mockRequest({}, "", "webauthn_reg_challenge=ch1"),
    );
    expect(res.status).toBe(401);
  });
  ...
  it("returns success on valid verification", async () => {
    const res = await webauthnRegVerifyRoutes.POST(
      mockRequest(
        { credential: { id: "cred-123" }, deviceName: "My Passkey" },
        "",
        "webauthn_reg_challenge=ch1",
      ),
    );
    expect(res.status).toBe(200);
    const body = await res.json();
