...
const { mockGetSession, mockPrisma, mockRequirePermission, mockRequireAuth, mockFormatDateTime } =
  vi.hoisted(() => {
    ...
    return {
      ...
      mockPrisma: {
        ...
        notification: deepModel({
          ...
          update: vi
            .fn()
            .mockResolvedValue(createMockNotification({ read: true, readAt: new Date() })),
          delete: vi.fn().mockResolvedValue({ id: "notif-1" }),
          groupBy: vi.fn().mockResolvedValue([{ type: "order", read: false, _count: 1 }]),
        ...
        notificationPreference: deepModel({
          ...
          upsert: vi
            .fn()
            .mockResolvedValue(createMockNotificationPreference({ emailOnOrder: false })),
        }),
        ...
        roleSetting: deepModel({
          ...
          update: vi.fn().mockResolvedValue(createMockRoleSetting({ allowed: false })),
          upsert: vi.fn().mockResolvedValue(
            createMockRoleSetting({
              id: "rs-3",
              role: "STAFF",
        ...
        webhookEndpoint: deepModel({
          ...
          update: vi.fn().mockImplementation(({ where_unused, data }) =>
            Promise.resolve({
              ...createMockWebhookEndpoint({
...
vi.mock("crypto", () => {
  ...
  const randomBytes = vi.fn((size: number) => Buffer.from("a".repeat(size)));
...
describe("API Keys", () => {
  ...
  describe("POST", () => {
    ...
    it("creates an API key and returns the raw key on creation", async () => {
      const res = await apiKeysRoutes.POST(mockRequest({ name: "My Key", permissions: "read" }));
      expect(res.status).toBe(200);
      const body = await res.json();
...
describe("Billing Plans (public, no auth guard)", () => {
...
describe("Billing Invoices", () => {
  ...
  it("respects limit query param up to 100", async () => {
    ...
    expect(mockPrisma.invoice.findMany).toHaveBeenCalledWith(expect.objectContaining({ take: 10 }));
  });
...
describe("Billing Subscription", () => {
  describe("authorization", () => {
    ...
    it("PUT returns 403 when requirePermission denies", async () => {
      mockRequirePermission.mockResolvedValue(permissionDenied());
      const res = await billingSubscriptionRoutes.PUT(mockRequest({ action: "cancel" }));
      expect(res.status).toBe(403);
    });
  });
  ...
  describe("POST", () => {
    ...
    it("returns 404 when plan not found", async () => {
      ...
      const res = await billingSubscriptionRoutes.POST(mockRequest({ planId: "plan-unknown" }));
      expect(res.status).toBe(404);
  ...
  describe("PUT", () => {
    ...
    it("returns 400 for invalid action", async () => {
      const res = await billingSubscriptionRoutes.PUT(mockRequest({ action: "invalid" }));
      expect(res.status).toBe(400);
    });
    ...
    it("reactivates cancelled subscription", async () => {
      const res = await billingSubscriptionRoutes.PUT(mockRequest({ action: "reactivate" }));
      expect(res.status).toBe(200);
      const body = await res.json();
      expect(body.id).toBe("sub-1");
      expect(body.cancelAtPeriodEnd).toBe(false);
...
describe("Notifications API", () => {
  describe("authorization", () => {
    ...
    it("POST returns 403 when requirePermission denies", async () => {
      mockRequirePermission.mockResolvedValue(permissionDenied());
      const res = await notificationsRoutes.POST(mockRequest({ type: "order", title: "Test" }));
      expect(res.status).toBe(403);
    });
  ...
  describe("PUT", () => {
    ...
    it("returns 400 for invalid action", async () => {
      const res = await notificationsRoutes.PUT(mockRequest({ id: "notif-1", action: "invalid" }));
      expect(res.status).toBe(400);
    });
  ...
  describe("DELETE", () => {
    ...
    it("clears all notifications when action is clear-all", async () => {
      const res = await notificationsRoutes.DELETE(mockRequest({ action: "clear-all" }));
      expect(res.status).toBe(200);
...
describe("Notifications Batch API", () => {
  it("returns 403 when requirePermission denies", async () => {
    mockRequirePermission.mockResolvedValue(permissionDenied());
    const res = await notificationsBatchRoutes.POST(mockRequest({ action: "mark-all-read" }));
    expect(res.status).toBe(403);
    expect(mockRequirePermission).toHaveBeenCalledWith(
      "update",
      "notifications",
  ...
  it("returns 400 for invalid action", async () => {
    const res = await notificationsBatchRoutes.POST(mockRequest({ action: "invalid" }));
    expect(res.status).toBe(400);
  });
  ...
  it("deletes all read notifications", async () => {
    const res = await notificationsBatchRoutes.POST(mockRequest({ action: "delete-all-read" }));
    expect(res.status).toBe(200);
    const body = await res.json();
...
describe("Notifications Preferences API", () => {
  describe("authorization", () => {
    ...
    it("PUT returns 403 when requirePermission denies", async () => {
      mockRequirePermission.mockResolvedValue(permissionDenied());
      const res = await notificationsPreferencesRoutes.PUT(mockRequest({ emailOnOrder: false }));
      expect(res.status).toBe(403);
    });
  });
...
describe("Roles API", () => {
  describe("authorization", () => {
    ...
    it("PUT returns 403 when requirePermission denies", async () => {
      mockRequirePermission.mockResolvedValue(permissionDenied());
      const res = await rolesRoutes.PUT(mockRequest({ id: "rs-1", allowed: false }));
      expect(res.status).toBe(403);
    });
...
describe("Webhooks API", () => {
  describe("authorization", () => {
    ...
    it("PUT returns 403 when requirePermission denies", async () => {
      mockRequirePermission.mockResolvedValue(permissionDenied());
      const res = await webhooksRoutes.PUT(mockRequest({ id: "wh-1", name: "Updated" }));
      expect(res.status).toBe(403);
    });
  ...
  describe("GET", () => {
    it("returns webhook endpoints with delivery counts", async () => {
      ...
      expect(body[0].subscribedEvents).toEqual(["order.created", "order.updated"]);
  ...
  describe("PUT", () => {
    ...
    it("returns 404 when webhook not found", async () => {
      ...
      const res = await webhooksRoutes.PUT(mockRequest({ id: "wh-unknown", name: "Updated" }));
      expect(res.status).toBe(404);
    });

    it("returns 400 when events contain invalid values", async () => {
      const res = await webhooksRoutes.PUT(mockRequest({ id: "wh-1", events: ["bad.event"] }));
      expect(res.status).toBe(400);
    });
...
describe("Webhook Deliveries API", () => {
  ...
  it("filters by endpointId when provided", async () => {
    await webhookDeliveriesRoutes.GET(mockRequest(undefined, "endpointId=wh-1"));
...
describe("Webhook Test API", () => {
  it("returns 403 when requirePermission denies", async () => {
    mockRequirePermission.mockResolvedValue(permissionDenied());
    const res = await webhookTestRoutes.POST(mockRequest({ endpointId: "wh-1" }));
    expect(res.status).toBe(403);
  ...
  it("returns 404 when webhook not found", async () => {
    mockPrisma.webhookEndpoint.findUnique.mockResolvedValueOnce(null);
    const res = await webhookTestRoutes.POST(mockRequest({ endpointId: "wh-unknown" }));
    expect(res.status).toBe(404);
  });
