...
import { BarChart3, ShoppingCart, Package, Lightbulb, Loader2, StopCircle } from "lucide-react";
      ...
      {suggestedQuestions.map((q) => {
        ...
        return (
          ...
          >
            <Icon
              size={14}
              className="h-3.5 w-3.5 text-emerald-500 shrink-0 group-hover:scale-110 transition-transform"
            />
...
function ThinkingIndicator() {
  return (
    <div className="flex items-start gap-3 px-4 py-3">
      <div className="flex items-center justify-center w-8 h-8 rounded-xl bg-gradient-to-br from-emerald-500 to-[oklch(0.78_0.22_147.35)] text-white shrink-0">
    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          >
            {/* Header */}
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-gray-800 bg-gradient-to-r from-emerald-50 to-green-50 dark:from-gray-800 dark:to-gray-800/80">
              <div className="flex items-center gap-3">
                <div className="flex items-center justify-center w-9 h-9 rounded-xl bg-gradient-to-br from-emerald-500 to-[oklch(0.78_0.22_147.35)] text-white shadow-lg shadow-emerald-500/20">
            ...
            <div className="flex-1 overflow-y-auto scrollbar-thin">
              {messages.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full px-6 text-center">
                  <div className="flex items-center justify-center w-14 h-14 rounded-2xl bg-gradient-to-br from-emerald-100 to-green-100 dark:from-emerald-900/30 dark:to-green-900/30 mb-4">
                    <BotIcon size={28} className="h-7 w-7 text-emerald-500" />
                  </div>
                  ...
                  {messages.map((message, i) => (
                    ...
                    >
                      {message.role === "assistant" && (
                        <div className="flex items-center justify-center w-8 h-8 rounded-xl bg-gradient-to-br from-emerald-500 to-[oklch(0.78_0.22_147.35)] text-white shrink-0 shadow-sm">
                          <BotIcon size={16} className="h-4 w-4" />
                        </div>
                      )}
                      ...
                      {message.role === "user" && (
                        <div className="flex items-center justify-center w-8 h-8 rounded-xl bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 shrink-0 shadow-sm">
