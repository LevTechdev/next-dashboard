...
const Anthropic = (props: any) => (
  <svg viewBox="0 0 24 24" fill="currentColor" {...props}>
    <path d="M12 2L2 22h3l2.5-5h9l2.5 5h3L12 2zm0 4.5l3.5 7h-7l3.5-7z" />
  </svg>
);
const AnthropicDark = Anthropic;
const DeepSeek = (props: any) => (
  <svg viewBox="0 0 24 24" fill="currentColor" {...props}>
...
const Google = (props: any) => (
  <svg viewBox="0 0 24 24" fill="currentColor" {...props}>
    <path d="M12.545,10.239v3.821h5.445c-0.712,2.315-2.647,3.972-5.445,3.972c-3.332,0-6.033-2.701-6.033-6.032s2.701-6.032,6.033-6.032c1.498,0,2.866,0.549,3.921,1.453l2.814-2.814C17.503,2.988,15.139,2,12.545,2C7.021,2,2.534,6.486,2.534,12s4.487,10,10.011,10c8.396,0,10.249-7.85,9.426-11.748L12.545,10.239z" />
  </svg>
);
const MistralAI = (props: any) => (
  <svg viewBox="0 0 24 24" fill="currentColor" {...props}>
    <path d="M4 4h4v16H4zm12 0h4v16h-4zM10 4h4v8h-4z" />
  </svg>
);
...
function AIInput_Voice() {
  ...
  // Bar heights for the "listening" visual. Generated once per listening pass
  // and kept in state: calling Math.random() in render is impure, which made
  // the bars jitter on every unrelated state change (and disagree between the
  // server and the client — an unpatched hydration diff).
  const [bars, setBars] = useState<number[] | null>(null);

  useEffect(() => {
    setBars((prev) =>
      submitted ? (prev ?? Array.from({ length: 48 }, () => 20 + Math.random() * 80)) : null,
    );
  }, [submitted]);
