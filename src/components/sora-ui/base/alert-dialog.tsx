...
function AlertDialog({
  ...
  const contextValue = useMemo(() => ({ open: Boolean(open), setOpen }), [open, setOpen]);
 ...
 */
function AlertDialogTrigger({ className, render, ...props }: AlertDialogTriggerProps) {
 ...
 */
function AlertDialogOverlay({ className, render, ...props }: AlertDialogOverlayProps) {
  const prefersReducedMotion = useReducedMotion();
...
type AlertDialogViewportProps = ComponentProps<typeof AlertDialogPrimitive.Viewport>;

/**
 * Optional scroll viewport wrapper for overflowing alert dialog content.
 */
function AlertDialogViewport({ className, ...props }: AlertDialogViewportProps) {
  return (
        ...
        <AlertDialogPrimitive.Portal keepMounted>
          ...
          >
            <AlertDialogPrimitive.Popup
              className={cn(
                "group/alert-dialog-content pointer-events-auto relative grid w-full max-w-[calc(100%-2rem)] gap-4 rounded-2xl sm:rounded-xl border border-border bg-background p-6 text-foreground text-sm shadow-2xl outline-none data-[size=sm]:max-w-xs data-[size=sm]:p-4 data-[size=default]:sm:max-w-lg data-[size=sm]:sm:max-w-sm",
                className,
              )}
              data-size={size}
...
function AlertDialogFooter({ className, ...props }: AlertDialogFooterProps) {
  return (
    <div
      className={cn(
        "mt-2 flex flex-col-reverse gap-2 rounded-b-2xl sm:rounded-b-xl border-border border-t bg-muted/40 p-4 group-data-[size=default]/alert-dialog-content:-mx-6 group-data-[size=sm]/alert-dialog-content:-mx-4 group-data-[size=default]/alert-dialog-content:-mb-6 group-data-[size=sm]/alert-dialog-content:-mb-4 group-data-[size=sm]/alert-dialog-content:grid group-data-[size=sm]/alert-dialog-content:grid-cols-2 sm:flex-row sm:justify-end",
        className,
      )}
...
function AlertDialogAction({
  ...
  return (
    <Button className={className} data-slot="alert-dialog-action" variant={variant} {...props} />
  );
}
 ...
 */
function AlertDialogClose({ className, render, ...props }: AlertDialogCloseProps) {
  const prefersReducedMotion = useReducedMotion();
